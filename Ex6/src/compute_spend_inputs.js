...
function computeInput(depth, transcript, nullifier) {
    // TODO
    // 创建一个新的 SparseMerkleTree
    let tree = new SparseMerkleTree(depth);
    let nonce;
    ...
    for (let item of transcript) {
        if (item.length === 1) {
            ...
            // 如果有两个元素（nullifier 和 nonce）
            if (item[0] === nullifier) {
                // 找到目标 nullifier，保存对应的 nonce
                nonce = item[1];
            }
            // 插入 nullifier 和 nonce 的哈希值
            tree.insert(mimc2(item[0], item[1]));
        }
    }
    ...
    // 获取到目标叶子节点的路径
    let path = tree.path(mimc2(nullifier, nonce));
